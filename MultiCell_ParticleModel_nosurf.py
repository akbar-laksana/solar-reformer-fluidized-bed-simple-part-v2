...
def solarReact_SolverFun(SV):       
    ...
    (Yk_p, Xk_p) = solarReact_SinglePartUnpack(SV, ind, gas, BedParams, SurfParams)
    
    # Particle model governing equations
    partmod_res, _, _, _, _ = solarReact_PartModEqs_MultiCellPart(gas, gas_surf, surf, GasParams, PartParams, BedParams, SurfParams, ind,  \
...
SurfParams['tau']           = 4.0                                              # [-] tortuosity of the particle 
SurfParams['Rpore']         = 300e-9                                              # [m] pore diameter 
SurfParams['R_Ni']          = 20e-9
...
ind = {
    ...
    'tot': ((gas['kspec'] ) * SurfParams['n_p']) * BedParams['n_y'],
    'start': np.arange(BedParams['n_y']) * ((gas['kspec'] ) * SurfParams['n_p'])
}
...
for i_p in range(SurfParams['n_p']):
    for i_spec in range(gas['kspec']):
            ind['Yk_p'][i_spec + (i_p * (gas['kspec'] ))] = \
                    partmod_vars_start + i_spec
...
for j_y in range(BedParams['n_y']):
    ...
    for i_p in range(SurfParams['n_p']):
        ...
        if BedParams['kinetics'] == 2:
            ...
            gas['obj'].equilibrate('TP')
            surf['obj'].advance_coverages(1.5e1, 1e-7, 1e-14, 1e-0, 5e8, 20)    
...
#%% Unpack and plot 
(Yk_p_sol, Xk_p_sol) = solarReact_SinglePartUnpack(Solution['x'], ind, gas, BedParams, SurfParams)
