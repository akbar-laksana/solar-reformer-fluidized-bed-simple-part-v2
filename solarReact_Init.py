...
def solarReact_Init(gas, gas_surf, part, wall, surf, GasParams, PartParams, BedParams, WallParams, SurfParams, EnvParams):
    ...
    if PartParams['interface'] == 1:
        ind['partmod_varnames'] = ['Yk_p_int', 'Yk_p']
    else:
        ind['partmod_varnames'] = ['Yk_p']
    ind['partmod_vars'] = len(ind['partmod_varnames'])
    
    # Total number of partmod variables (minus all gas species)
    partmod_vars_start  = ind['reactormod_vars'] - 1
    
    # Set array of Yk_p indices
    if PartParams['interface'] == 1:
        ind['Yk_p_int'] = np.zeros(( gas['kspec'] ))
    ind['Yk_p'] = np.zeros(( gas['kspec'] ))
    
    # # Setting of variable indices
    for i_var in range(ind['partmod_vars']):
        if ind['partmod_varnames'][i_var] == 'Yk_p_int':
            ind['Yk_p_int'] = partmod_vars_start + i_var + 1 + np.arange(gas['kspec'] )
        if ind['partmod_varnames'][i_var] == 'Yk_p':
            if PartParams['interface'] == 1:
                ind['Yk_p'] = partmod_vars_start + i_var + gas['kspec'] + np.arange(gas['kspec'] )
            else:
                ind['Yk_p'] = partmod_vars_start + i_var + 1 + np.arange(gas['kspec'] )
                
    # Add N-1 species ONCE/TWICE to the total number of reactormod variables
    if PartParams['interface'] == 1:
        ind['partmod_vars'] = (ind['partmod_vars'] + (gas['kspec'] - 1) + (gas['kspec'] - 1))
    else:
        ind['partmod_vars'] = (ind['partmod_vars'] + (gas['kspec'] - 1))
    ...
    if initialize_T == 'constant':
        T_bg = np.ones((BedParams['n_y'])) * (GasParams['T_in'] + T_bg_out)/2
    ...
    for i_var in range(ind['reactormod_vars'] - (gas['kspec'] - 1)):
        ...
        if var_name == 'T_we':  
            ...
            boundsLow[ind['start'] + ind['T_we']]       = (GasParams['T_in'] - 500) / scale['var'][ind['start'] + ind['T_we']]
            boundsUp[ind['start'] + ind['T_we']]        = 2500 / scale['var'][ind['start'] + ind['T_we']]
        ...
        # Fluidized Bed Side Wall Temperature    
            ...
            boundsLow[ind['start'] + ind['T_wb']]       = (GasParams['T_in'] - 500) / scale['var'][ind['start'] + ind['T_wb']]
            boundsUp[ind['start'] + ind['T_wb']]        = 2500 / scale['var'][ind['start'] + ind['T_wb']]
        ...
        # Particle Solid Temperature    
            ...
            boundsLow[ind['start'] + ind['T_bs']]      = (GasParams['T_in'] - 500) / scale['var'][ind['start'] + ind['T_bs']]
            boundsUp[ind['start'] + ind['T_bs']]       = 2500 / scale['var'][ind['start'] + ind['T_bs']]
        ...
        # Fluidizing Gas Temperature    
            ...
            boundsLow[ind['start'] + ind['T_bg']]       = (GasParams['T_in'] - 500) / scale['var'][ind['start'] + ind['T_bg']]
            boundsUp[ind['start'] + ind['T_bg']]        = 2500 / scale['var'][ind['start'] + ind['T_bg']]
    ...
    for i_var in range( len(ind['partmod_varnames']) ):
        ...
        if var_name == 'Yk_p_int':  # Gas Mass Fraction within the Particle
            for i_spec in range(gas['kspec'] ):
                ...
                boundsUp[ind['start'] + ind['Yk_p_int'][i_spec]]        = 1.0 / scale['var'][ind['start'] + ind['Yk_p_int'][i_spec]]
                
                # res_2 denotes scaling when restart is activated
                scale['res_2'][ind['start'] + ind['Yk_p_int'][i_spec]]    = scale['res_2_Yk_p_int'] * abs(GasParams['mdot_in'])
                
        elif var_name == 'Yk_p':  # Gas Mass Fraction within the Particle
            for i_spec in range(gas['kspec'] ):
                ...
                # res_2 denotes scaling when restart is activated
                scale['res_2'][ind['start'] + ind['Yk_p'][i_spec]]  = scale['res_2_Yk_p'] * abs(GasParams['mdot_in'])
